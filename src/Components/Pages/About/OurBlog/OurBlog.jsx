...
const BlogCard = ({ image, category, title, date, symbol: Symbol, id }) => {
    return (
        <Link to={`/blogs/${id}`} className="transform transition-transform duration-300 hover:scale-105">
        <div className="bg-white rounded-lg shadow-md overflow-hidden px-1 relative w-full hover:shadow-lg transition-shadow duration-300">
            <img
                src={image}
                alt={title}
                className="w-[95%] mx-auto rounded-xl h-48 object-cover md:h-64 transition-transform duration-300 hover:scale-105"
            />
            <div className="p-4 md:p-6">
                ...
                <div className="flex mt-10 md:flex-row lg:flex-row justify-between md:items-center">
                    ...
                    <div>
                        <Link
                            to={`/blogs/${id}`}
                            className="text-black hover:text-[#990A05] hover:underline font-medium text-sm flex items-center transition-colors duration-300"
                        >
                            Read more <RiArrowRightSLine className="text-xl ml-2 transition-transform transform hover:translate-x-1" />
                        </Link>
                    </div>
                </div>
            </div>
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-0">
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between mb-8">
                ...
                <Link className="text-gray-600 hover:text-gray-800 text-sm font-semibold transition-colors duration-300" to={'/blogs'}>
